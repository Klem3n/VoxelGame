...
public class Player extends InputAdapter {
    ...
    public void update (float deltaTime) {
        ...
        if (keys.containsKey(FORWARD)) {
            tmp.set(camera.direction).nor().scl(deltaTime * velocity).y = 0;
        ...
        if (keys.containsKey(STRAFE_LEFT)) {
            tmp.set(camera.direction).crs(camera.up).nor().scl(-deltaTime * velocity).y = 0;
            camera.position.add(tmp);
        }
        ...
        World.INSTANCE.get(getPosition().cpy().add(0f, -1.6f, 0f));
